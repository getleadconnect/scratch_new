...
namespace App\Http\Controllers\GlScratch;
...
class HyundaiScratchController extends Controller
{
    ...
    */
        ...
        if ($validator->passes()) 
        {
            ...
            try
            {
                ...
                if ($user) {
                    ...
                    return response()->json(['message'=> 'Successfully listed','offers'=>$offers,'path'=>url('/'), 'status' => 'success']);
                }else{
    ...
    public function sendOtp(Request $request)
    {
        ...
        $check_num = ScratchCustomers::where('vchr_billno', $request->bill_no)->where('vchr_mobno',$last10Digits)->where('fk_int_user_id',$userid)->first();
        if($check_num){
    ...
    public function scratchBranches(){
        $rule=[ 
            'user_id' => 'required',
            ];
    ...
    public function scratchCustomer(){
        $input = request()->all();
        $rule = [
            'user_id' => 'required',
            'campaign_id' => 'required',
            ...
            try
            {
                if($user) 
                {
                    ...
                    if(request('bill_no') != "" && request('type_id') == 1)
                    {
                        ...
                        if(request()->has('bill_no')){
                            $check_bill = ScratchCustomers::where('vchr_billno', request('bill_no'))->where('fk_int_user_id',request('user_id'))->first();
                            if($check_bill){
